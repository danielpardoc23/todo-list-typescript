...
for (let i = 0; i < list.length; i++){
    let newTask: Task = new Task(list[i].name, list[i].state, list[i].priority, list[i].description);
...
function addTask(){
    ...
    let prioridad = ((document.getElementById("prioridad") as HTMLInputElement).value as any as number);
    ...
    if(valido){
        ...
        let task: Task = new Task(nombre, estado, prioridad, descripcion);
        classList.push(task);
...
function updateState(name){
    for(let i = 0; i < classList.length; i++){
        if(name === classList[i].name){
            let nuevoEstado = (document.getElementById(name) as HTMLInputElement).value;
...
function lessPriority(name: string){
    for(let i = 0; i < classList.length; i++){
        if(classList[i].name === name){
            let newPriority = classList[i].priority + 1;
            classList[i].priority = newPriority;
...
function drop(ev) {
    ...
    let nuevoEstado = (document.getElementById(tarea).parentNode as HTMLInputElement).id;
